...
class Landlord {
	...
	//@Ensures(houses_owned.size() = houses_owned.size() + 1)
	...
	public void select_tenants(String occupation_choice, char gender_choice, boolean need_vegan) {
		for(Entry<Tenant, House> e: tenant_house_req_map.entrySet())
		{
			...
			if ((t.getOccupation().equals(occupation_choice) && t.getGender() == gender_choice) && (need_vegan == t.is_tenant_vegan() || need_vegan == false))
			{
				...
				System.out.println("Tenant " + t.getName() + " selected by landlord " + this.name + " for house " + h.get_house_num());
			}
			
			else
			{
				System.out.println("Tenant " + t.getName() + " rejected by landlord " + this.name + " for house " + h.get_house_num());
			}
		}
	...
	public boolean house_roommates_match(House house)
	{
		if(!house.isHouse_empty()) //if tenant(s) already present then check if preferences match
		{
			...
			//	System.out.println("current tenant preferences: " + curr_tenant_preferences[0] + " " + curr_tenant_preferences[1]);
				...
				{
					System.out.println("Tenants " + t.getName() + " and " + this.getName() + " do not match for " + house.get_house_num());
					return false;
				}
			}
		}
...
class LegalAdvisor
{
	...
	public boolean is_legal(House h)
	{
		...
		{
			System.out.println(this.name + " claims that the lease for house " + h.get_house_num() + " is illegal!");
			return false;
		}
		
		System.out.println(this.name + " certifies that the lease for house " + h.get_house_num() + " is legal!");
		return true;
	}
...
public class Main {
		...
		System.out.println("Rent for house " + house1.get_house_num() + " is " + house1.getRent() + " and safety_index is " + house1.getSafety_index());
		l1.register_house(house2);
		System.out.println("Rent for house " + house2.get_house_num() + " is " + house2.getRent() + " and safety_index is " + house2.getSafety_index());
...
//LANDLORDS HAVE REGISTERED HOUSES
		...
		if(houses_selected_for_t1.size() > 0)
		{
			for(House h : houses_selected_for_t1)
			{
				...
				t1.occupy_house(h);
				System.out.println("Tenant " + t1.getName() + " occupied house " + h.get_house_num() + " owned by " + House.house_landlord_map.get(h.get_house_num()).get_name());
				break;
		...
		if(t1.get_house_occupied() != null)
		{
			...
			System.out.println("tenant list before sublease");
			...
			System.out.println("updated tenant list after sublease");
		...
		{
			System.out.println("Review for house " + e.getKey() + ": " + e.getValue() );
		}
